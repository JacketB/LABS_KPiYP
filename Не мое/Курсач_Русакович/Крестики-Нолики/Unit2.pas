...
var
  Game: TGame;
  Tag: integer;
  player1, player2:string; //переменные имён игрока
  sscore1,sscore2:integer; //переменные счета игроков
implementation

{$R *.dfm}
uses Unit1, Unit3, Unit4, Unit5;
procedure winlose; //процедура проверки победы
begin
//!!!!Каждый ряд аналогичен друг к другу, за исключением переменных Label(в программе Cell1-9)!!!!
//Первый ряд
if (Game.Cell1.Caption <> '') and  (Game.Cell2.Caption <> '') and  (Game.Cell3.Caption <> '') then //если ряд не пустой
 if (Game.Cell1.Caption = Game.Cell2.Caption) and  (Game.Cell2.Caption = Game.Cell3.Caption) then begin //если ряд из одинаковых
   if (Game.Cell1.Caption = ' X') and  (Game.Cell2.Caption = ' X') and  (Game.Cell3.Caption = ' X') then begin  //если первый в ряду равен иксу
   sscore1 := sscore1+1; //ко счёту первого игрока добавляем 1
   Game.score1.Caption := IntToStr(sscore1); //выводим счёт
   MessageWin.Message.Caption :=player1; //добавляем в форму с сообщением имя первого игрока
   MessageWin.Show; //выводим форму с сообщением
   Game.N2.Click; //Кнопка "Новая игра", т.е. очищает игровое поле
   end else begin //если первый ряд неравен иксу (т.е. равен нулю)
   sscore2 := sscore2+1; //ко счёту второго игрока добавляем 1
   Game.score2.Caption := IntToStr(sscore2); //выводим счёт
   MessageWin.Message.Caption := player2; //добавляем в форму с сообщением имя второго игрока
   MessageWin.Show; //выводим форму с сообщением
   Game.N2.Click; //Кнопка "Новая игра", т.е. очищает игровое поле
   end;
 end;
...
if (Game.Cell1.Caption <> '') and  (Game.Cell2.Caption <> '') and  (Game.Cell3.Caption <> '') and  (Game.Cell4.Caption <> '')and  (Game.Cell5.Caption <> '') and  (Game.Cell6.Caption <> '') and  (Game.Cell6.Caption <> '') and  (Game.Cell7.Caption <> '') and  (Game.Cell8.Caption <> '') and  (Game.Cell9.Caption <> '') then begin
  MessageWin.Message.Caption := 'ДРУЖБА';
  MessageWin.Show;
  Game.N2.Click;
end;
...
begin
 Game.Timer1.Enabled:=false;  //Для надежности выключаем таймер
 Game.Cell1.Caption:='';      //Убираем заголовок первого лэйбла
 Game.Cell2.Caption:='';      //Убираем заголовок второго лэйбла
 Game.Cell1.Caption:='';      //Убираем заголовок третьего лэйбла
 Game.Cell4.Caption:='';      //Убираем заголовок четвертого лэйбла
 Game.Cell5.Caption:='';      //Убираем заголовок пятого лэйбла
 Game.Cell6.Caption:='';      //Убираем заголовок шестого лэйбла
 Game.Cell7.Caption:='';      //Убираем заголовок седьмого лэйбла
 Game.Cell8.Caption:='';      //Убираем заголовок восьмого лэйбла
 Game.Cell9.Caption:='';      //Убираем заголовок девятого лэйбла
 Game.Tag:=0;
 Game.Timer1.Enabled:=true;   //Возобновляем работу таймера
end;

//!!!!Все элементы имеет одинаковый код, за исключением названия лэйблов!!!!
procedure TGame.Cell1Click(Sender: TObject); //первый элемент
begin
if Cell1.Caption<>'' then exit; //Проверка на заголовок панели (анти-мухлеж)
 if Tag=0
  then //Меняем tag формы (для чередования X и O)
   begin
    Cell1.Font.Color:=Setting.colorX.Font.Color; //присваиваем цвет Х-у с формы настроек
    Cell1.Caption:=' X'; //если таг = 0 тогда вписываем X
    Tag:=1;
   end
  else
   begin
    Cell1.Font.Color:=Setting.colorO.Font.Color; //присваиваем цвет О-у с формы настроек
    Cell1.Caption:=' O'; //если таг = 1 тогда вписываем O
    Tag:=0;
   end;
   winlose; //процедура проверки на победу
end;

procedure TGame.Cell2Click(Sender: TObject); //второй элемент
begin
...
var pname1, pname2:string; //Переменные для ник-нейма
i:integer; //переменная для цикла
begin
sscore1:=0; //при создании счёт обнуляется
sscore2:=0; //при создании счёт обнуляется
pname1:=UpperCase(Login.player1Edit.Text+':'); //Добавляем ник с первой формы в переменную и в верхний регистр
playername1.Caption:=pname1; //выводим на форму
pname2:=UpperCase(':'+Login.player2Edit.Text); //Добавляем ник с первой формы в переменную и в верхний регистр
playername2.Caption:=pname2; //выводим на форму
for i := 2 to pname2.Length do player2:=player2+pname2[i]; //цикл прохода по имени второго игрока, без ":"
label2.Caption:=player2; //записываем в переменную
for i := 1 to pname1.Length-1 do player1:=player1+pname1[i]; //цикл прохода по имени второго игрока, без ":"
label1.Caption:=player1; //записываем в переменную
end;


procedure TGame.N2Click(Sender: TObject); //кнопка "Новая игра"
begin
Cell1.Caption:=''; //очистка первого лэйбла
Cell2.Caption:=''; //очистка второго лэйбла
Cell3.Caption:=''; //очистка третьего лэйбла
Cell4.Caption:=''; //очистка четвертого лэйбла
Cell5.Caption:=''; //очистка пятого лэйбла
Cell6.Caption:=''; //очистка шестого лэйбла
Cell7.Caption:=''; //очистка седьмого лэйбла
Cell8.Caption:=''; //очистка восьмого лэйбла
Cell9.Caption:=''; //очистка девятого лэйбла
end;
procedure TGame.N3Click(Sender: TObject); //кнопка "Закрыть"
begin
Login.Close(); //закрывает ОСНОВНУЮ форму, следовательно, и всю программу
end;

procedure TGame.N4Click(Sender: TObject); //кнопка "О программе"
begin
Info.Show(); //открывает форму информации
end;

procedure TGame.N5Click(Sender: TObject); //кнопка "Настройки"
begin
  Setting.Show(); //открывает форму с настройками
end;

procedure TGame.N6Click(Sender: TObject); //кнопка "Сменить имя"
begin
player1:=''; //очистка имени первого игрока
player2:=''; //очистка имени второго игрока
